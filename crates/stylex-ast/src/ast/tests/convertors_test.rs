...
fn convert_member_prop_to_string_numeric_renders_shortest_decimal() {
  // A computed numeric key is looked up by `String(Number)`, so the rendering
  // has to be JS's spelling — exponential outside the fixed-notation range —
  // and not merely a decimal that parses back to the same `f64`.
  assert_eq!(
...
/// The rendering itself is covered by `stylex_utils::number`; this pins that a
/// numeric literal is rendered through it rather than through Rust's `Display`.
#[test]
fn convert_number_to_js_string_matches_js_string_number() {
  let cases: &[(f64, &str)] = &[(42.5, "42.5"), (1e21, "1e+21"), (1e-7, "1e-7")];

  for (value, expected) in cases {
    assert_eq!(
      convert_number_to_js_string(&Number {
        span: DUMMY_SP,
        value: *value,
        raw: None,
      }),
...
#[test]
fn convert_number_to_js_string_non_finite() {
  let non_finite: &[(f64, &str)] = &[
    (f64::NAN, "NaN"),
    (f64::INFINITY, "Infinity"),
    (f64::NEG_INFINITY, "-Infinity"),
  ];

  for (value, expected) in non_finite {
    assert_eq!(
      convert_number_to_js_string(&Number {
        span: DUMMY_SP,
        value: *value,
