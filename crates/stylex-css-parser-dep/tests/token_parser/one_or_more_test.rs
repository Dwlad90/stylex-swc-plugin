#[cfg(test)]
mod one_or_more {

  use cssparser::Token;
  use stylex_css_parser_dep::{token_parser::TokenParser, tokens::TokenType};

  #[test]
  fn parses_one_or_more() {
    let string_parser = TokenParser::<Token<'static>>::get_token_parser(TokenType::Ident);
    let whitespace_parser = TokenParser::<Token<'static>>::get_token_parser(TokenType::Whitespace);

    let parser = TokenParser::one_or_more(
      string_parser
        .map(|t| t, None)
        .where_fn(|t| *t == Token::Ident("foo".into()), None),
    )
    .separated_by(whitespace_parser)
    .to_parser();

    assert_eq!(
      parser.parse_to_end("foo").unwrap(),
      vec![Token::Ident("foo".into())]
    );

    assert_eq!(
      parser.parse_to_end("foo foo").unwrap(),
      vec![Token::Ident("foo".into()), Token::Ident("foo".into())]
    );

    assert_eq!(
      parser.parse_to_end("foo foo foo").unwrap(),
      vec![
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into())
      ]
    );
    assert_eq!(
      parser.parse_to_end("foo foo foo foo").unwrap(),
      vec![
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into())
      ]
    );
    assert_eq!(
      parser.parse_to_end("foo foo foo foo foo").unwrap(),
      vec![
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into()),
        Token::Ident("foo".into())
      ]
    );
  }

  #[test]
  #[should_panic(expected = r#"Expected OneOrMore<Ident.map().flatMap()> but got Never"#)]
  fn fails_to_parse_a_different_string() {
    let string_parser = TokenParser::<Token<'static>>::get_token_parser(TokenType::Ident);
    let whitespace_parser = TokenParser::<Token<'static>>::get_token_parser(TokenType::Whitespace);

    let parser = TokenParser::one_or_more(
      string_parser
        .map(|t| t, None)
        .where_fn(|t| *t == Token::Ident("foo".into()), None),
    )
    .separated_by(whitespace_parser)
    .to_parser();

    parser.parse_to_end("bar").unwrap();
  }
}
