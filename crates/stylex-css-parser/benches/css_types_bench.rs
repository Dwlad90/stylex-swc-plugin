...
These benchmarks mirror the JavaScript benchmarks in packages/style-value-parser/__benchmarks__/
to ensure our Rust implementation meets or exceeds JavaScript performance.

Mirrors: packages/style-value-parser/__benchmarks__/
*/
...
fn custom_ident_benchmarks(c: &mut Criterion) {
    ...
    for input in &ident_inputs {
        group.bench_with_input(format!("parse_custom_ident_{}", input), input, |b, input| {
            b.iter(|| {
