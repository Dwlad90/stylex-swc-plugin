...
Mirrors the token parsing functionality tested in JavaScript benchmarks.
*/
...
fn combinator_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("flat_map_chaining", |b| {
        b.iter(|| {
            let parser = TokenParser::always(black_box(5))
                .flat_map(|x| TokenParser::always(x * 3), Some("triple"));
            black_box(parser.parse("test"))
        })
    });
    ...
    group.bench_function("where_clause_filtering", |b| {
        b.iter(|| {
            let parser = TokenParser::always(black_box(10))
