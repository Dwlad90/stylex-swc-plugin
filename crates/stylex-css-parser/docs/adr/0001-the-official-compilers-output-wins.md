...
identical and much shorter — and costs two of the six class names.
...
When the upstream report is resolved, this compiler follows. Cross-reporting is
tracked by ticket 11 of `.scratch/fix_media-query-order-wrapping/`; the report
numbers belong in this paragraph once they exist.
...
single query. Do not delete it as arbitrary; the measured curve behind it is in
`.scratch/fix_media-query-order-wrapping/evidence/expansion-cost.md`.
...
**The expansion is the performance baseline, not a regression to chase.** A
hundred-rung ladder costs the same 2.7 seconds as a twenty-four-rung one,
because the deepest thing either contains is a twenty-rung sub-ladder. Any
future fast path has to beat that number and justify its own correctness
separately.
...
parenthesis would otherwise hide is on the only path there is. Three grammar
differences remain where this compiler accepts what upstream refuses; they are
named in ticket 13 rather than closed here.
