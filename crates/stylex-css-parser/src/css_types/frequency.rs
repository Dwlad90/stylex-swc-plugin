...
Mirrors: packages/style-value-parser/src/css-types/frequency.js
*/
...
/// Mirrors: Frequency class in frequency.js
...
impl Display for Frequency {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    if self.unit == "Hz" && self.value >= 1000.0 && self.value % 1000.0 == 0.0 {
...
mod tests {
  ...
  fn test_frequency_display() {
    ...
    assert_eq!(hertz.to_string(), "440Hz");
    ...
    // Should not convert if not a whole kilohertz
    let partial = Frequency::new(1500.0, "Hz".to_string());
    assert_eq!(partial.to_string(), "1500Hz");
  }
