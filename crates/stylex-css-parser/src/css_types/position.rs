 ...
 *
 * Functionally equivalent to JavaScript position.js implementation.
 * Covers all major CSS position parsing scenarios while working with Rust paradigms.
 */
use crate::token_parser::TokenParser;
use crate::token_types::SimpleToken;
use std::fmt;

/// Horizontal position keywords - matches JavaScript HorizontalKeyword exactly
...
/// Vertical position keywords - matches JavaScript VerticalKeyword exactly
#[derive(Debug, Clone, PartialEq)]
...
/// Horizontal position value - matches JavaScript Horizontal type exactly
...
impl fmt::Display for Horizontal {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Horizontal::KeywordWithOffset(k, lp) => write!(f, "{} {}", k, lp), // JavaScript: array.join(' ')
    }
  }
}

/// Vertical position value - matches JavaScript Vertical type exactly
...
impl fmt::Display for Vertical {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Vertical::KeywordWithOffset(k, lp) => write!(f, "{} {}", k, lp), // JavaScript: array.join(' ')
    }
  }
}

/// CSS Position - matches JavaScript Position class exactly
#[derive(Debug, Clone, PartialEq)]
pub struct Position {
...
impl Position {
  ...
  /// Parser for horizontal keywords - matches JavaScript horizontalKeyword exactly
  ...
  /// Parser for vertical keywords - matches JavaScript verticalKeyword exactly
  ...
  /// Parser for Position - implements the essential JavaScript functionality
  ...
  ///
  /// This achieves functional equivalence with the JavaScript parser
  ...
  pub fn parser() -> TokenParser<Position> {
    ...
    // This matches the JavaScript priority order
...
impl fmt::Display for Position {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    // Matches JavaScript toString() exactly:
