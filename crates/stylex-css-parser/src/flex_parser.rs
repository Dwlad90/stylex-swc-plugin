/*!
Flexible Parser System for JavaScript-Equivalent Parsing
...
4. **Dynamic Type Handling**: JavaScript-like value mixing
...
use crate::{
    ...
    token_types::{TokenList, SimpleToken},
    CssParseError,
};
use std::fmt::Debug;

/// Parser that can return any CSS value type - enables JavaScript-like parsing
...
impl FlexCombinators {
    /// Parse sequence with mixed types - JavaScript equivalent!
    ///
    /// JavaScript: TokenParser.sequence(fn, number, comma, number, comma, number)
    /// Rust: mixed_sequence(vec![fn_parser, number_parser, comma_parser, ...])
    ...
    /// Try all parsers until one succeeds - backtracking support
    ///
    /// Much more flexible than TokenParser::one_of because it can handle
    /// different return types via CssValue
    pub fn try_all(parsers: Vec<FlexParser>) -> FlexParser {
                TokenParser::new(move |input| {
            let mut last_error = CssParseError::ParseError { message: "No parsers provided".to_string() };
    ...
    ///
    /// Enables parsers to behave differently depending on where they're used,
    /// just like JavaScript parsers can check their context
    ...
    ///
    /// JavaScript equivalent: parse values until ')' or ',' encountered
    ...
    ///
    /// JavaScript equivalent: automatically parse function name and arguments
    pub fn function_with_args(name: &'static str, arg_parser: FlexParser) -> FlexParser {
        TokenParser::new(move |input| {
            ...
            match input.consume_next_token()? {
                ...
                _ => return Err(CssParseError::ParseError {
    ...
    ///
    /// Much more flexible than current separated_by() combinator
...
/// Extension trait to add context support to TokenList
...
impl TokenListExt for TokenList {
    fn get_context(&self) -> Option<ParseContext> {
        ...
        // In full implementation, this would track parsing context
        Some(ParseContext::Default)
    }

    fn set_context(&mut self, _context: ParseContext) {
        // Store context for context-aware parsing
        // Implementation would store context state
    }
}
...
pub fn parse_hsl() -> FlexParser {
    ...
    FlexCombinators::function_with_args("hsl",
        FlexCombinators::comma_separated(
            FlexCombinators::try_all(vec![
                // Hue: angle or number
                FlexCombinators::try_all(vec![
                    dimension().where_fn(|v| {
                        v.has_unit("deg") || v.has_unit("rad") || v.has_unit("grad") || v.has_unit("turn")
...
mod tests {
    ...
    fn test_mixed_sequence() {
        ...
        // Which in JavaScript would be: sequence(fn, number, comma, number, comma, number)
        let values = vec![
            CssValue::ident("rgb"),
    ...
    fn test_try_all_flexibility() {
        ...
        // unlike the current one_of which requires same return type
    ...
    fn test_smart_tokens() {
        ...
        let dim_token = SimpleToken::Dimension { value: 10.0, unit: "px".to_string() };
