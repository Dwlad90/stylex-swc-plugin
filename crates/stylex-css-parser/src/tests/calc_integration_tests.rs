...
use crate::css_types::{
  Addition, Calc, CalcDimension, CalcValue, Division, Group, Multiplication, Percentage,
  Subtraction,
};
...
mod calc_tests {
  ...
  fn test_parses_dimension_values() {
    // Test calc(20px)
    let calc_px = Calc::new(CalcValue::Dimension(CalcDimension::new(
      20.0,
      "px".to_string(),
    )));
    ...
    // Test calc(2em)
    let calc_em = Calc::new(CalcValue::Dimension(CalcDimension::new(
      2.0,
      "em".to_string(),
    )));
    ...
    // Test calc(1.5rem)
    let calc_rem = Calc::new(CalcValue::Dimension(CalcDimension::new(
      1.5,
      "rem".to_string(),
    )));
  ...
  fn test_parses_addition_operations() {
    // Test calc(10 + 5)
    let add_op = Addition::new(CalcValue::Number(10.0), CalcValue::Number(5.0));
  ...
  fn test_parses_subtraction_operations() {
    // Test calc(10 - 5)
    let sub_op = Subtraction::new(CalcValue::Number(10.0), CalcValue::Number(5.0));
  ...
  fn test_parses_multiplication_operations() {
    // Test calc(10 * 5)
    let mul_op = Multiplication::new(CalcValue::Number(10.0), CalcValue::Number(5.0));
  ...
  fn test_parses_division_operations() {
    // Test calc(10 / 2)
    let div_op = Division::new(CalcValue::Number(10.0), CalcValue::Number(2.0));
  ...
  fn test_parses_nested_operations_with_parentheses() {
    // Test calc((10 + 5) * 2)
    let inner_add = Addition::new(CalcValue::Number(10.0), CalcValue::Number(5.0));
    let group = Group::new(CalcValue::Addition(inner_add));
    let outer_mul = Multiplication::new(CalcValue::Group(group), CalcValue::Number(2.0));
  ...
  fn test_to_string_round_trips() {
    let test_cases = vec![
      ...
      Calc::new(CalcValue::Dimension(CalcDimension::new(
        20.0,
        "px".to_string(),
      ))),
