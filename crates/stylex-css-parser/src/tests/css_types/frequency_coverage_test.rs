// Additional coverage tests for frequency.rs.
// Targets:
//   - the `else { None }` arm for an invalid unit inside `extract_frequency_token`
//     (reachable by calling the function directly with an invalid-unit Dimension)
//   - the `else { None }` arm for a non-Dimension token inside
//     `extract_frequency_token` (unreachable through the public parser, which
//     guarantees a Dimension token, but coverable via the named function)
