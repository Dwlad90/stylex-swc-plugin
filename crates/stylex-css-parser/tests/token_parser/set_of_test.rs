...
mod set_of {
  ...
  //     None,
  //   );
  //   panic!("Revert commented separated_by above");
  ...
  fn makes_separators_optional_for_optional_parsers() {
    ...
    let parser = TokenParser::set_of(vec![
      ...
      // TokenParser::<Token<'static>>::string("bar").map(Some, None),
    ...
    assert_eq!(
      ...
      vec![
        Some(Some(Token::Ident("foo".into()))),
        Some(Some(Token::Ident("bar".into()))),
        Some(Some(Token::Ident("baz".into())))
      ]
    );

    assert_eq!(
      parser.parse_to_end("foo baz bar").unwrap(),
      vec![
         Some(Some(Token::Ident("foo".into()))),
         Some(Some(Token::Ident("bar".into()))),
         Some(Some(Token::Ident("baz".into())))
