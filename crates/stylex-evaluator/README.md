...
Pure utility functions extracted from the StyleX JS expression evaluator and
common utilities.
...
- `is_invalid_method`, `is_mutating_object_method`, `is_mutating_array_method`
  -- Method validation
...
`native_functions.rs`) remains in `stylex-transform` due to deep `StateManager`
coupling. This crate contains only the pure helper functions that were
extractable without code changes.
