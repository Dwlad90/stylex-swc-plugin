...
**Root dir**:
The `unstable_module_resolution.rootDir` option. Distinct from the root path and
used for one thing only: rewriting Turbopack's `/ROOT/` placeholder inside an
aliased path. With no root dir, a `/ROOT/` alias cannot resolve at all.
...
symlinked package. Resolution prefers it when it exists so two versions of one
...
`exports` subset the resolver supports rather than the whole file.
