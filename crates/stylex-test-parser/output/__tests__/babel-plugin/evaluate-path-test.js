...
describe('custom path evaluation works as expected', ()=>{
    ...
    test('Evaluates Simple Arrays and Objects', ()=>{
        ...
        expect(evaluateFirstStatement('const x = {name: "Name", age: 43};', {})).toEqual({
            name: 'Name',
            age: 43
        });
        expect(evaluateFirstStatement('const x = [];', {})).toEqual([]);
    ...
    test('Evaluates Objects with spreads', ()=>{
        expect(evaluateFirstStatement('const x = {name: "Name", ...({hero: true}), age: 43};', {})).toEqual({
            name: 'Name',
            hero: true,
            age: 43
        });
        expect(evaluateFirstStatement('const x = {name: "Name", ...({name: "StyleX", age: 1}), age: 43};', {})).toEqual({
            name: 'StyleX',
            age: 43
        });
    });
    ...
    test('Evaluates custom functions', ()=>{
        ...
        expect(evaluateFirstStatement('const x = makeArray(1, 2, 3);', {
            identifiers: {
                makeArray: {
                    fn: makeArray
                }
        ...
        expect(evaluateFirstStatement('const x = stylex.makeArray(1, 2, 3);', {
            memberExpressions: {
                stylex: {
                    makeArray: {
                        fn: makeArray
    ...
    test('Evaluates custom functions that return non-static values', ()=>{
        ...
        expect(evaluateFirstStatement('const x = makeClass("Hello");', {
            identifiers: {
                makeClass: {
                    fn: makeClass
                }
    ...
    test('Evaluates custom functions used as spread values', ()=>{
        ...
        expect(evaluateFirstStatement('const x = {name: "Name", ...makeObj("Hello"), age: 30};', {
            identifiers: {
                makeObj: {
                    fn: makeObj
                }
    ...
    test('Evaluates custom functions that take paths', ()=>{
        ...
        expect(evaluateFirstStatement('const x = getNode("Hello");', {
            identifiers: {
                getNode: {
                    fn: getNode,
                    takesPath: true
    ...
    describe('evaluating function expressions', ()=>{
        ...
        test('Array map', ()=>{
            expect(evaluateFirstStatement('const x = [1, 2, 3].map(x => x * 2);', {})).toEqual([
                2,
                4,
                6
            ]);
        ...
        test('Array filter', ()=>{
            expect(evaluateFirstStatement('const x = [1, 2, 3].filter(x => x % 2 === 0);', {})).toEqual([
                2
            ]);
        });
        test('Array map and filter', ()=>{
            expect(evaluateFirstStatement('const x = [1, 2, 3].map(x => x * 2).filter(x => x % 2 === 0);', {})).toEqual([
                2,
                4,
                6
            ]);
        ...
        test('Object entries', ()=>{
            expect(evaluateFirstStatement('const x = Object.entries({a: 1, b: 2, c: 4}).filter((entry) => entry[1] % 2 === 0);', {})).toEqual([
                [
                    'b',
                    2
                ],
            ...
            expect(evaluateFirstStatement('const x = Object.fromEntries(Object.entries({a: 1, b: 2, c: 4}).filter((entry) => entry[1] % 2 === 0));', {})).toEqual({
                b: 2,
                c: 4
            });
        });
        test('Methods called by string should be bind', ()=>{
            expect(evaluateFirstStatement('const x = "".concat("10px"," ").concat("10px");', {})).toBe('10px 10px');
    ...
    describe('evaluate-path prototype-chain escape prevention', ()=>{
        ...
        test('blocks walking to Function via .constructor.constructor', ()=>{
            expect(evaluateFirstStatement('const x = ({}).constructor.constructor;', {})).toEqual({
                confident: false
            });
        });
        test('blocks arbitrary code execution via Function constructor', ()=>{
            expect(evaluateLastStatement('({}).constructor.constructor("return 1").call({});', {})).toEqual({
                confident: false
            });
        });
        test('blocks constructor calls on allowlisted globals', ()=>{
            expect(evaluateLastStatement('Object.constructor("return 1").call({});', {})).toEqual({
                confident: false
            });
        });
      ...
      `;
            expect(evaluateLastStatement(code, {
                identifiers: {},
                memberExpressions: {}
            })).toEqual({
                confident: false
        ...
        test('blocks __proto__ traversal', ()=>{
            expect(evaluateLastStatement('({}).__proto__.constructor.constructor("return 1").call({});', {})).toEqual({
                confident: false
            });
        });
        ...
        test('blocks .constructor off a string literal', ()=>{
            expect(evaluateLastStatement('"abc".constructor.constructor("return 1").call({});', {})).toEqual({
                confident: false
            });
        });
      ...
      `;
            expect(evaluateLastStatement(code, {})).toEqual({
                confident: false
            });
        });
      ...
      `;
        ...
        test('blocks Function reached through inherited config properties', ()=>{
            expect(evaluateLastStatement('constructor.constructor("return 1").call({});', {
                identifiers: {},
                memberExpressions: {}
            })).toEqual({
                confident: false
            });
            expect(evaluateLastStatement('valueOf.constructor("return 1").call({});', {
                identifiers: {},
                memberExpressions: {}
            })).toEqual({
                confident: false
        ...
        test('blocks reflective Object methods', ()=>{
            expect(evaluateFirstStatement('const x = Object.getPrototypeOf({});', {})).toEqual({
                confident: false
            });
            expect(evaluateFirstStatement("const x = Object.getOwnPropertyDescriptor({a: 1}, 'a');", {})).toEqual({
                confident: false
            });
            expect(evaluateFirstStatement('const x = Object.create({});', {})).toEqual({
                confident: false
            });
            expect(evaluateFirstStatement('const x = Object.setPrototypeOf({}, {});', {})).toEqual({
                confident: false
            });
        });
        test('still allows built-in methods that return plain data', ()=>{
            expect(evaluateFirstStatement('const x = Object.getOwnPropertyNames({a: 1});', {})).toEqual([
                'a'
            ]);
            expect(evaluateFirstStatement('const x = Object.isFrozen({});', {})).toBe(false);
            expect(evaluateFirstStatement('const x = Object.isExtensible({});', {})).toBe(true);
        });
        test('still deopts on non-deterministic and mutating methods', ()=>{
            expect(evaluateFirstStatement('const x = Math.random();', {})).toEqual({
                confident: false
            });
            expect(evaluateFirstStatement('const x = Object.assign({}, {a: 1});', {})).toEqual({
                confident: false
            });
            expect(evaluateFirstStatement('const x = Object.freeze({a: 1});', {})).toEqual({
                confident: false
            });
        });
