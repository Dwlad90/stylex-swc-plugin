...
describe('Evaluation of imported values works based on configuration', ()=>{
    describe('Theme name hashing based on fileName alone works', ()=>{
        ...
        stylex(styles.red);
      `);
            const expectedVarName = `var(--${options.classNamePrefix}${hash('otherFile.stylex.js//MyTheme.foreground')})`;
            expect(expectedVarName).toMatchInlineSnapshot(`"var(--__hashed_var__1jqb1tb)"`);
      ...
      `);
        ...
        import 'otherFile.stylex';
        import { MyTheme } from 'otherFile.stylex';
        _inject2(".__hashed_var__15x39w1{--__hashed_var__1jqb1tb:var(----__hashed_var__1jqb1tb)}", 1);
        const styles = {
          color: color => [{
