                    ...
                    path.traverse({
                        CallExpression (path, _state) {
                            if (path.node.callee.type === 'Identifier' && path.node.callee.name === 'inline') {
                                ...
                                path.replaceWith(t.memberExpression(stylesIdentifier, t.identifier(`$${index}`)));
                            }
                        }
                    });
                    path.node.body.push(t.variableDeclaration('const', [
                        t.variableDeclarator(stylesIdentifier, t.callExpression(t.memberExpression(t.identifier('stylex'), t.identifier('create')), [
                            t.objectExpression(collectedStyles.map((style, index)=>t.objectProperty(t.identifier(`$${index}`), style)))
                        ]))
                    ]));
                }
            }
