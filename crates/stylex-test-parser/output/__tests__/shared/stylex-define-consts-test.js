var themeName = 'TestTheme.stylex.js//buttonTheme';
...
describe('styleXDefineConsts', ()=>{
    test('returns correct structure for basic constants', ()=>{
        ...
        expect(injectableStyles[getConstHash('sm', constants.sm)].constKey).not.toBe(injectableStyles[getConstHash('md', constants.md)].constKey);
    });
    test('handles special characters in keys', ()=>{
        ...
        expect(injectableStyles[getConstHash('max-width', constants['max-width'])].constKey).not.toBe(injectableStyles[getConstHash('font-size*large', constants['font-size*large'])].constKey);
    });
    test('handles numeric keys', ()=>{
        ...
        expect(injectableStyles[getConstHash('1', constants[1])].constKey).not.toBe(injectableStyles[getConstHash('2', constants[2])].constKey);
    });
    test('generates consistent hashes for identical constants', ()=>{
        const constants = {
            padding: '10px'
        };
        const [jsOutput1, styles1] = styleXDefineConsts(constants, {
            themeName
        });
        const [jsOutput2, styles2] = styleXDefineConsts(constants, {
            themeName
        });
    ...
    test('generates different hashes for different constants', ()=>{
        ...
        const [_, styles1] = styleXDefineConsts(constants1, {
            themeName
        });
        const [__, styles2] = styleXDefineConsts(constants2, {
            themeName
        });
    ...
    test('preserves object keys', ()=>{
        ...
        const [jsOutput, _] = styleXDefineConsts(constants, {
            themeName
        });
        expect(Object.keys(jsOutput)).toEqual([
            'borderRadius',
            'colorPrimary'
        ]);
    });
