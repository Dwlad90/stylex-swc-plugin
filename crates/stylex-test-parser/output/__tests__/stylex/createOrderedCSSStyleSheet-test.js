...
var removeStyleElement = (element)=>{
    ...
    describe('#insert', ()=>{
      ...
      `);
        ...
        test('insertion of CSSMediaRule within CSSConditionRule', ()=>{
            const sheet = createOrderedCSSStyleSheet();
            sheet.insert('@supports (display:grid) { ' + '@media (min-width:320px) { ' + '.selector { color: red } ' + '} ' + '}', 0);
            sheet.insert('@container main (max-width: 850px) { ' + '@media (min-width:320px) { ' + '.selector { color: red } ' + '} ' + '}', 0);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
        "[stylesheet-group="0"]{}
    ...
    describe('#update', ()=>{
      ...
      `);
        ...
        test('updates a media query rule', ()=>{
            const sheet = createOrderedCSSStyleSheet();
            sheet.insert('@media (min-width: 768px) { .responsive { display: flex } }', 6);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
        "[stylesheet-group="6"]{}
        @media (min-width: 768px) { .responsive { display: flex } }"
      `);
            sheet.update('@media (min-width: 768px) { .responsive { display: flex } }', '@media (min-width: 1024px) { .responsive { display: grid } }', 6);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
"[stylesheet-group="6"]{}
...
`);
      ...
      `);
            sheet.update('.box { width: 10px; height: 10px; color: red }', '.box { width: 20px; height: 20px; color: blue }', 3);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
        "[stylesheet-group="3"]{}
      ...
      `);
        ...
        test('updates nested at-rules', ()=>{
            const sheet = createOrderedCSSStyleSheet();
            sheet.insert('@media (min-width: 768px) { @supports (display: grid) { .nested { display: grid } } }', 9);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
        "[stylesheet-group="9"]{}
        @media (min-width: 768px) { @supports (display: grid) { .nested { display: grid } } }"
      `);
            sheet.update('@media (min-width: 768px) { @supports (display: grid) { .nested { display: grid } } }', '@media (min-width: 1024px) { @supports (display: flex) { .nested { display: flex } } }', 9);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
"[stylesheet-group="9"]{}
...
`);
            sheet.update('.selector:not(#\\#):not(#\\#):not(#\\#) { color: red }', '.selector:not(#\\#):not(#\\#):not(#\\#) { color: blue }', 3);
            expect(sheet.getTextContent()).toMatchInlineSnapshot(`
"[stylesheet-group="3"]{}
...
`);
    ...
    describe('client-side hydration', ()=>{
        ...
        test('from SSR CSS', ()=>{
            ...
            serverSheet.insert('@media (min-width: 320px) { .four-1 { color: red; } }', 4);
            serverSheet.insert('@supports (display: grid) { ' + '@media (min-width:320px) { ' + '.five { color: red; } ' + '} ' + '}', 5);
...
`);
            clientSheet.insert('@media (min-width: 320px) { ' + '.four-1 { color: red; } ' + '}', 4);
            expect(clientSheet.getTextContent()).toEqual(hydratedTextContent);
            clientSheet.insert('@media (min-width: 320px) { ' + '.four-2 { color: blue; } ' + '}', 4);
