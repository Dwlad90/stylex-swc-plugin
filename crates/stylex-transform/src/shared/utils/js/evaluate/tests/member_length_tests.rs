...
/// Parses one expression and evaluates it. Panics propagate: a refusal that
/// aborts the build instead of deopting is the defect
/// [#1265](https://github.com/Dwlad90/stylex-swc-plugin/issues/1265) reported,
/// and every refusal below sits one operand away from that seam.
...
/// Asserts the source folds to a number. Spelled as an exact count rather than
/// "some number", because the whole failure mode this file guards is a
/// confident answer that is not the right one.
#[track_caller]
fn assert_folds_to_number(source: &str, expected: f64) {
  ...
  match result.value {
    Some(EvaluateResultValue::Expr(Expr::Lit(Lit::Num(num)))) => {
...
#[track_caller]
fn assert_deopt_names_property(source: &str, property: &str) {
  let result = evaluate_source(source);
