...
Formerly named `stylex-misc`, this crate provides deterministic
floating-point hashing and SWC helper functions shared by several
higher-level crates in the compiler pipeline.

- **Deterministic hashing** — `hash_f64` converts `f64` values to a
  stable, hashable representation
- **SWC helpers** — factory for a default `ExprCtx` used in
...
- **Depended on by**: `stylex-ast`, `stylex-rs-compiler`,
...
| Module | Description |
| ------ | ----------- |
| `hash` | `hash_f64` — deterministic floating-point hashing utility |
| `swc` | `get_default_expr_ctx` — creates a default SWC `ExprCtx` for expression type checking |
