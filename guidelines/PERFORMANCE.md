...
`benchmark/budget.json` is `pending-calibration`: reports, never blocks, until
ceilings are seeded from repeated clean runs using a robust upper bound plus
headroom. Ceilings are valid only on the canonical environment
(`x86_64-unknown-linux-gnu`, Node 24.18.0, recorded runner image); drift fails
as recalibration rather than comparing.
