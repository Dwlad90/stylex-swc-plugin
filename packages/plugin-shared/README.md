...
- `stylex-loader` — runs the `@stylexswc/rs-compiler` Rust transform on JS/TS
  modules, stores extracted rules on `module.buildInfo` (webpack transport), and
  appends a context-relative dummy CSS import carrying the rules in its query
  (rspack transport + HMR invalidation).
...
The two bundlers move extracted rules from the loader to the plugin differently:
...
- **webpack**: the loader writes rules to `module.buildInfo`; the plugin
  ...
  in webpack's filesystem cache, so cached rebuilds (where loaders don't re-run)
  keep their CSS.
- **rspack**: loaders can't persist `module.buildInfo` across the native
  boundary, so the plugin re-parses rules from the dummy-import queries embedded
  in chunk module identifiers on every compilation. Identifiers survive caching
  and rebuilding the map drops rules of deleted files.
