...
function dfsProcessQueries(
  ...
  Object.entries(obj).forEach(([key, value]) => {
    if (typeof value === 'object' && value !== null) {
      console.log('!!!!!! ~ media-query-transform.js:63 ~ dfsProcessQueries ~ key, value:', key, value);
      result[key] = dfsProcessQueries(value, depth + 1);
    } else {
      result[key] = value;
    }
  });
  console.log('!!!!!! ~ media-query-transform.js:70 ~ dfsProcessQueries ~ result:', {result, depth});
  ...
  if (
    ...
    for (let i = mediaKeys.length - 1; i > 0; i--) {
      ...
      console.log('!!!!!! ~ media-query-transform.js:84 ~ dfsProcessQueries ~ mediaQuery:', mediaQuery);
    ...
    for (let i = 0; i < mediaKeys.length; i++) {
      ...
      console.log('!!!!!! ~ media-query-transform.js:96 ~ dfsProcessQueries ~ baseMediaQuery:', baseMediaQuery);
