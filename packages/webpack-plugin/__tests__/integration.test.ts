...
function createConfig(mode: 'development' | 'production'): Configuration {
  return {
    ...
    plugins: [new StyleXPlugin({ rsOptions: { dev: false } }), new MiniCssExtractPlugin()],
  };
}
...
describe('@stylexswc/webpack-plugin integration', () => {
  ...
  test('second build from a cold filesystem cache still emits the full CSS', async () => {
    ...
    const withCache = (): Configuration => ({
      ...createConfig('production'),
      cache: {
        type: 'filesystem',
        cacheDirectory,
        // keep the cache self-contained for the test
        buildDependencies: {},
      },
    });

    const firstCss = getStylexCss(await compile(withCache()));

    expect(firstCss).not.toBeNull();

    // A fresh compiler restores modules (and their buildInfo) from the
    // filesystem cache without re-running loaders — the rules must survive.
    const secondCss = getStylexCss(await compile(withCache()));

    expect(secondCss).not.toBeNull();

    for (const declaration of BASIC_FIXTURE_EXPECTED_CSS) {
      expect(secondCss).toContain(declaration);
    }
  });
});
