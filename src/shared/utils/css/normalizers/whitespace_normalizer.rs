use crate::shared::regex::{
    CSS_RULE_REGEX, WHITESPACE_NORMALIZER_BRACE_REGEX, WHITESPACE_NORMALIZER_MULTIPLYER_REGEX,
};
...
pub(crate) fn whitespace_normalizer(result: String) -> String {
    ...
    let normalized_css_string =
        WHITESPACE_NORMALIZER_MULTIPLYER_REGEX.replace_all(css_string, " $1 $2");

    let normalized_css_string = WHITESPACE_NORMALIZER_BRACE_REGEX
